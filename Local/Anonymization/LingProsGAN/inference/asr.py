...
class InferenceASR:
    ...
    def __init__(self, model_name, results_dir, data_dir, model_dir, device):
        ...
        model_dir = os.path.join(model_dir, 'asr', model_name)
    ...
    def recognize_speech(self, path_to_ad:str):
        speech = self.preprocess(path_to_ad)
        nbests = self.speech2text(speech)
        text, *_ = nbests[0]
        text = '~' + text + '~#'  # add pause at beginning and end of utterance
        return text

    def recognize_speech_old(self, dataset, utterance_list=None):

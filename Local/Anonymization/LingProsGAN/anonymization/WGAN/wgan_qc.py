...
class WassersteinGanQuadraticCost:
    ...
    def __init__(self, generator, discriminator, gen_optimizer, dis_optimizer, criterion, epochs, n_max_iterations, opt_iterations,
    ...
    def _critic_deep_regression_(self, images, opt_iterations=1000):
        # To update critic per batch.
        # Total loss is a weighted sum of the MSEloss and 
        # the quadratic transport loss
    ...
    def _train_epoch(self, data_loader, writer=None, experiment=None):
        for i, data in enumerate(tqdm(data_loader)):
            try:
                images = data[0]
                speaker_ids = data[1] # dont think this is ever needed
                self.num_steps += 1
                # self.tensorboard_counter += 1
                if self.gen_steps >= self.n_max_iterations:
                    return
                self._critic_deep_regression_(images, self.opt_iterations)
                ...
                print(f'trouble with batch {i}')
                pass

    def train(self, data_loader, writer=None, experiment=None):
        self.G.train()
        self.D.train()
